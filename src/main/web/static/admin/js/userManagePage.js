...
function select() {
    ...
    let cid = $('#cid').val();
    let sex = $('#select').val();
    let url = "/admin_admin_userManage?";
...
function ajax(name, pageNum, u) {
    ...
    // 声明url, 是否是查询
    let p = {}
        ,url = "/admin_admin_userManage?start="+pageNum+"&name="+name;
    if (u !== undefined && u !== "") {
        url = u+"&start="+pageNum;
    }
    $.ajax({
...
function add() {
    javaex.dialog({
        ...
        url : "/pages/admin/href/userAdd.jsp",	// 页面地址或网址或请求地址
        width : "800",	// 弹出层宽度
        height : "500",	// 弹出层高度
...
function edit(id) {
    javaex.dialog({
        ...
        title: "修改用户",
        id : "add",	// 指定id，仅当页面存在多个弹出层，需要关闭指定弹出层时才使用
        url : "/admin_admin_editUser?id="+id,	// 页面地址或网址或请求地址
        width : "800",	// 弹出层宽度
        height : "500",	// 弹出层高度
        isClickMaskClose: true
    });
...
function dele(id) {
    ...
    javaex.alert({
        content : "确定删除",
        callback : "deleback('"+id+"')"
    });
}
// 删除指定用户
function deleback(id) {
    callback(id);
}
// 删除选中用户
...
function callback(idArray) {
    $.ajax({
        url: "/admin_admin_userDelete?idArray="+idArray.toString(),
        type: "post",
        async: false,
        dataType: "text",
        success: function (data) {
